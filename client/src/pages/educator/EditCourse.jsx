...
const EditCourse = () => {
  ...
  const [courseDescription, setCourseDescription] = useState(''); // Add this state
  ...
  const fetchCourseData = async () => {
    try {
      ...
      if (data.success) {
        ...
        setCourseDescription(course.courseDescription || ''); // Set description
        setLoading(false);
      } else {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const courseData = {
        courseTitle,
        courseDescription: quillRef.current ? quillRef.current.root.innerHTML : courseDescription,
      ...
      // console.log('Sending PUT request to:', `${backendUrl}/api/educator/course/${id}`);

      const { data } = await axios.put(`${backendUrl}/api/educator/course/${id}`, formData, {
        headers: { 
  ...
  useEffect(() => {
    // Set the course description when data is loaded and Quill is initialized
    if (quillRef.current && courseDescription && !loading) {
                    ...
                    {chapter.chapterContent?.map((lecture, lectureIndex) => (
                      <div key={lecture.lectureId || lectureIndex} className="flex justify-between items-center mb-2">
                        <span>
                          ...
                          <span className="ml-1">{lecture.isPreviewFree ? 'Free Preview' : 'Paid'}</span>
                        </span>
                        <img 
